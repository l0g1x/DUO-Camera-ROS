...
const std::string DUODense3DDriver::CameraNames[TWO_CAMERAS] = {"left","right"};
...
void DUODense3DDriver::fillDUOImages(sensor_msgs::Image& leftImage, sensor_msgs::Image& rightImage, const PDUOFrame pFrameData)
{
...
bool DUODense3DDriver::validateCameraInfo(const sensor_msgs::Image &image, const sensor_msgs::CameraInfo &ci)
{
	return (ci.width == image.width && ci.height == image.height);
}


void DUODense3DDriver::publishImages(const sensor_msgs::ImagePtr image[TWO_CAMERAS])
{
...
void CALLBACK DUODense3DCallback(const PDUOFrame pFrameData, void *pUserData)
{

	// Using singleton to access DUODense3DDriver
...
bool DUODense3DDriver::initializeDUO()
{
...
void DUODense3DDriver::dynamicCallback(duo3d_ros::DuoConfig &config, uint32_t level)
{
...
void DUODense3DDriver::startDUO()
{
	// If we could successfully open the DUO, then lets start it to finish
	// the initialization
	ROS_INFO("Starting DUO...");
	StartDUO(_duoInstance, DUODense3DCallback, NULL);
	ROS_INFO("DUO Started.");
}


void DUODense3DDriver::shutdownDUO()
{
